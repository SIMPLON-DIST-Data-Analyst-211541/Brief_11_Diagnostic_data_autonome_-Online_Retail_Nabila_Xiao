...
│
├── main.py                       # Point d'entrée unique (Script de contrôle simplifié avec 'os')
...
Top 10 des clients réels les plus importants en volume d'achat.

Top 10 des clients effectuant le plus de retours (utilisateurs atypiques ou insatisfaits).

c)Analyse Temporelle :

Évolution du chiffre d'affaires cumulé mois par mois pour détecter les pics de saisonnalité commerciale (ex: fêtes de fin d'année).

d)Analyse Géographique :

Détermination du Top 5 des pays générant le plus de revenus (concentration des ventes hors marché domestique du Royaume-Uni).


🚀 Installation et Lancement
Prérequis
Assurez-vous d'avoir Python 3.12+ installé, ainsi que les packages requis. Vous pouvez les installer via votre terminal :
